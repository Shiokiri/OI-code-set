...
class CH {
    protected:
   	...
   	public:
		...
		P getPointById(int id) { return p[id]; }
		double diamt() {
			double ret = 0;
			if(n == 2) { return dis(p[0], p[1]); }
			for(int i = 0, j = 2; i < n; i++) {
				while(sgn(det(p[(i + 1) % n] - p[i], p[j] - p[i]) - 
				det(p[(i + 1) % n] - p[i], p[(j + 1) % n] - p[i])) < 0)
				{
					j = (j + 1) % n;
				}
				ret = std::max(ret, std::max(dis(p[i], p[j]), dis(p[(i + 1) % n], p[j])));
			}
			return ret;
		}
};
...
int main()
{
	...
	CH ch = graham(n, p);
	cout << (long long)(sqr(ch.diamt())) << endl;
	return 0;
}
